...
public class FileUtil {
    ...
    public static void copyFile(String source, String dest) throws IOException {
        if(Files.exists(Paths.get(source))) {
            ...
            log.error("Not copied, not found");
        }
    }
    ...
    public static int processHlsToFlv(String streamName, Long idLive) {

        ProcessExecutor plsof = null;
        int exitCode = 0;
        try {
            log.info("Sending to FLV process {}", streamName);
            plsof = new ProcessExecutor().command("/opt/encoder/transcoderflv.sh", streamName);

            //exitCode = plsof.waitFor();

            RMemoryAPI.getInstance()
                    .pushSetElemToMemory(Constants.LIVE_KEY + "hlstoflv_queue", idLive +"_"+ Constants.REGION);

            exitCode = plsof.destroyOnExit().execute().getExitValue();

            RMemoryAPI.getInstance()
                    .delFromSetElem(Constants.LIVE_KEY + "hlstoflv_queue", idLive +"_"+ Constants.REGION);
    ...
    public static int copyDir(String src, String dest) {
        ...
        try {
            log.info("Copy dir {}", cmd);
            output = new ProcessExecutor().command(cmd)
                    .destroyOnExit()
                    .execute().getExitValue();

        } catch (Exception ex) {
            log.error("ERROR IN TRYING copy DIR", ex);
        }
    ...
    public static String copyVideoFile(String src, String dest) {

        String output = "";
        List<String> cmd = new ArrayList<>();

        cmd.add("/opt/encoder/copyvideo.sh");

        cmd.add(src);
        cmd.add(dest);

        try {
            log.info("Copy file {}", cmd);
            output = new ProcessExecutor().command(cmd)
                    .destroyOnExit()
                    .readOutput(true).execute()
                    .outputUTF8();
            ...
            log.error("ERROR IN TRYING copy FILE", ex);
        }
